...
(function () {
  ...
  w.yellowSegments = (s.yellowSegments && s.yellowSegments.length > 0) ? s.yellowSegments : (DEFAULT_WEIGHTS.yellowSegments || null);
    ...
    w.pullPerBonus = (s.pullPerBonus != null) ? s.pullPerBonus : (DEFAULT_WEIGHTS.pullPerBonus != null ? DEFAULT_WEIGHTS.pullPerBonus : 450);
    w.pullPerBonusAmount = (s.pullPerBonusAmount != null) ? s.pullPerBonusAmount : (DEFAULT_WEIGHTS.pullPerBonusAmount != null ? DEFAULT_WEIGHTS.pullPerBonusAmount : 500);
  ...
  function buildSettingsModal(defaults, onSave, ignoreSaved) {
    ...
    pullC6FormRow.appendChild(pc6Label('每'));
    var pullC6PullPerBonusInput = pc6Input(w.pullPerBonus != null ? w.pullPerBonus : (DEFAULT_WEIGHTS.pullPerBonus != null ? DEFAULT_WEIGHTS.pullPerBonus : 450), '1', '#f59e0b', '每N抽加一次固定金额（0=不加）');
    pullC6FormRow.appendChild(pullC6PullPerBonusInput);
    pullC6FormRow.appendChild(pc6Label('抽+'));
    var pullC6PullBonusAmountInput = pc6Input(w.pullPerBonusAmount != null ? w.pullPerBonusAmount : (DEFAULT_WEIGHTS.pullPerBonusAmount != null ? DEFAULT_WEIGHTS.pullPerBonusAmount : 500), '1', '#4ade80', '每次加多少元');
    pullC6FormRow.appendChild(pullC6PullBonusAmountInput);
    pullC6FormRow.appendChild(pc6Label('元'));
    ...
    function updatePullC6Preview() {
      ...
      var pullPerB = parseFloat(pullC6PullPerBonusInput.value);
      var pullBonusAmt = parseFloat(pullC6PullBonusAmountInput.value);
      if (!isNaN(pullPerB) && pullPerB > 0 && !isNaN(pullBonusAmt)) {
        html += '；每' + pullPerB + '抽+' + pullBonusAmt + '元';
      }
      pullC6Preview.innerHTML = html;
    }
    [pullC6BaseInput, pullC6BaseBonusInput, pullC6StepInput, pullC6StepBonusInput, pullC6ThresholdInput, pullC6MaxWCInput, pullC6PullPerBonusInput, pullC6PullBonusAmountInput].forEach(function(inp) {
    ...
    loadPullC6DefaultBtn.onclick = function () {
      ...
      pullC6PullPerBonusInput.value = DEFAULT_WEIGHTS.pullPerBonus != null ? DEFAULT_WEIGHTS.pullPerBonus : 450;
      pullC6PullBonusAmountInput.value = DEFAULT_WEIGHTS.pullPerBonusAmount != null ? DEFAULT_WEIGHTS.pullPerBonusAmount : 500;
      updatePullC6Preview();
    };
    ...
    saveBtn.onclick = function () {
      ...
      var _pullC6PPBVal = parseFloat(pullC6PullPerBonusInput.value);
      newW.pullPerBonus = isNaN(_pullC6PPBVal) ? (DEFAULT_WEIGHTS.pullPerBonus != null ? DEFAULT_WEIGHTS.pullPerBonus : 450) : _pullC6PPBVal;
      var _pullC6PBAVal = parseFloat(pullC6PullBonusAmountInput.value);
      newW.pullPerBonusAmount = isNaN(_pullC6PBAVal) ? (DEFAULT_WEIGHTS.pullPerBonusAmount != null ? DEFAULT_WEIGHTS.pullPerBonusAmount : 500) : _pullC6PBAVal;
